...
describe('TextOperation', () => {
  ...
  it(
    ...
    h.randomTest(n, () => {
      ...
      expect(str.length).toBe(o.baseLength);
      const newAttributes: TextOp['attributes'][] = [];
  ...
  it(
    ...
    h.randomTest(n, () => {
      ...
      expect(operation.equals(TextOperation.fromJSON(operation.toJSON()))).toBe(
        true
      );
    })
  );
  ...
  it('FromJSON', () => {
    ...
    function assertIncorrectAfter(fn: (arg0: any[]) => any) {
  ...
  it('FromJSONWithAttributes', () => {
    ...
    function assertIncorrectAfter(fn: any) {
      const ops2 = ops.slice(0);
      fn(ops2);
      expect(() => {
        TextOperation.fromJSON(ops2);
  ...
  it(
    ...
    h.randomTest(2 * n, () => {
      ...
      expect(a.shouldBeComposedWith(b)).toBe(
        bInv.shouldBeComposedWithInverted(aInv)
      );
    })
  );
  ...
  it(
    ...
    h.randomTest(n, () => {
      ...
      const abPrime = a.compose(bPrime);
      const baPrime = b.compose(aPrime);
      const afterABPrimeAttributes: TextOp['attributes'][] = [];
      const afterAbPrime = abPrime.apply(
        str,
        attributes,
        afterABPrimeAttributes
      );
      const afterBaPrimeAttributes: TextOp['attributes'][] = [];
      const afterBaPrime = baPrime.apply(
        str,
        attributes,
        afterBaPrimeAttributes
      );
      expect(abPrime.equals(baPrime)).toBe(true);
      expect(afterAbPrime).toBe(afterBaPrime);
