...
export default class helpers {
  ...
  static randomAttributes(allowFalse?: boolean) {
    const attributes: TextOp['attributes'] = {};
    const count = helpers.randomInt(3);
    for (let i = 0; i < count; i++) {
      const name = attrNames[helpers.randomInt(attrNames.length)];
      const value =
        attrValues[helpers.randomInt(attrValues.length - (allowFalse ? 0 : 1))];
  ...
  static randomAttributesArray(n: number) {
  ...
  static randomOperation(str: string, useAttributes?: boolean) {
    ...
    while (true) {
      ...
      if (r < 0.2) {
        operation.insert(
          helpers.randomString(l),
          useAttributes ? helpers.randomAttributes() : {}
        );
      } else if (r < 0.4) {
        operation.delete(l);
      } else {
        operation.retain(
          l,
          useAttributes ? helpers.randomAttributes(/* allowFalse= */ true) : {}
        );
      }
    }
    if (Math.random() < 0.3) {
  ...
  static randomTest(n: number, fun: () => any) {
    return function() {
      while (n--) {
        fun();
      }
